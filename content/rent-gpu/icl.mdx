...
1. [Network Configuration](/rent-gpu/icl#1-network-configuration) 
2. [Version](/rent-gpu/icl#2-version-configuration)
3. [Services](/rent-gpu/icl#3-services-configuration)
4. [Profiles](/rent-gpu/icl#4-profiles)
5. [Deployment](/rent-gpu/icl#5-deployment-configuration)
6. [GPU Support](/rent-gpu/icl#6-gpu-integration-support-in-compute-profiles)
7. [Private Container Registry Integration](/rent-gpu/icl#7-private-container-registry-integration)
...
<Callout type="info">
  **NOTE:** For examples of deployment configurations:
  - [Single service deployment](https://github.com/spheronFdn/awesome-spheron/blob/main/examples/spheron.provider.yaml)
  - [Multi-service deployment](https://github.com/spheronFdn/awesome-spheron/blob/main/examples/spheron.multiservice.yaml)
  - [Persistent Storage deployment](https://github.com/spheronFdn/awesome-spheron/blob/main/examples/spheron.persistent.yaml)
  - [SSH deployment](https://github.com/spheronFdn/awesome-spheron/blob/main/examples/spheron.ssh.yaml)
  - [Fizz deployment](https://github.com/spheronFdn/awesome-spheron/blob/main/examples/spheron.ssh.yaml)
...
| expose     | No                 | Determines which entities are permitted to connect to the services. For additional information, see the [Port Exposure](/rent-gpu/icl#port-exposure) and [Port Range](/rent-gpu/icl#port-range) sections.                          |
...
## 4. Profiles
The profiles section is used to define named compute and placement profiles that can be referenced in the [deployment section](/rent-gpu/icl#5-deployment-configuration).
...
| compute    | Yes                | Specifies the compute resources that will be allocated to service instances. Refer to the [services.env](/rent-gpu/icl#compute-profiles) section for more details.                       |
| placement  | Yes                | Outlines the necessary attributes and the pricing configuration. See the [services.expose](/rent-gpu/icl#placement-profile) section for additional information.               |
...
Providers can only assign themselves to a predefined set of regions. If you specify an unlisted region, no provider will bid on your deployment, causing it to fail automatically. Please refer to the list mentioned in the [Supported Regions](/rent-gpu/supports#supported-regions) section.

<Callout type="info">
  To learn more about advanced placement attributes and fine-grained control over your deployments, check out the [Advanced Placement Attributes](/rent-gpu/icl#11-advanced-placement-attributes) section at the end of this guide.
</Callout>
...
For a comprehensive example of a GPU-enabled ICL, refer to this [example](https://github.com/spheronFdn/awesome-spheron/blob/main/examples/spheron.provider.yaml) which includes the declaration of several GPU models.
