...
# Deploy Your App on Spheron using CLI
...
### Get Some test tokens from the Team and ETH from the Faucet
...
4. Check if you got the ETH for gas and USDC tokens in your wallet:
```
sphnctl wallet balance --token USDC
```
This will show both USDC and ETH balances in your wallet.

<Callout type="info">
**Note:** 
- You won't get USDC, or uSPON tokens from the Spheron Faucet.
- You can get some credit token from the Spheron team if you are building something super amazing. 
...
1. Deposit USDC / uSPON / any other token to the escrow wallet:

```
sphnctl payment deposit --amount 10 --token USDC
```
...
2. Before deploying, you should set the private RPC URL by running the following command:
...
Please follow the documentation to obtain a custom base mainnet RPC URL: https://docs.spheron.com/user-guide/protocol-cli#setting-custom-base-mainnet-rpc-wss-url
