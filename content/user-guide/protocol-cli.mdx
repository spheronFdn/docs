...
You need to fund your wallet with some ETH to deploy anything on Spheron. We are currently deployed on Base Sepolia chain, you can try getting some ETH from the faucet or buy it from the market to pay for the transaction fees.
...
To set a custom RPC, you need to run this command:

```sh
sphnctl rpc set wss://[RPC URL]
```

To get a RPC URL, you can create app on Alchemy or QuickNode or Infura or any other RPC provider and get the RPC URL from there.

**Example:**

```sh
sphnctl rpc set wss://base-sepolia.g.alchemy.com/v2/your-api-key
```
