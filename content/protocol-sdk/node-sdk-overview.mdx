...
```javascript
const balance = await sdk.escrow.getUserBalance('USDC', '0xYourWalletAddress');
console.log('Your USDC balance in escrow is:', balance);
```

**Parameters:**

- `token` (string): The token symbol. Supported tokens are `USDC`, `uSPON`. Learn more about the tokens [here](/user-guide/supports#supported-payment-tokens).
...
- `token` (string): The token symbol to deposit (`USDC`, `uSPON` or any other token supported by the Spheron network).
...
- `token` (string): The token symbol to withdraw (`USDC`, `uSPON` or any other token supported by the Spheron network).
