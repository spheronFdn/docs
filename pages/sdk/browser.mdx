...
If you are using the `>=2.0.0` version of the `@spheron/browser-upload`
package, you also need to use the `>=2.0.0` version of the `@spheron/storage` on the server
side.
...
<Callout type="info">
  - For `>=2.0.0` versions, Please choose `storage` type in the dashboard while generating the Access Token.
  - For `<2.0.0` versions, Please choose `static site` type in the dashboard while generating the Access Token.
</Callout>
