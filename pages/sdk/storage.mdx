...
```js
const pinStatus: RootObject = await client.getCIDStatus(CID);
```
...
interface PeerData {
    ...
    ipfs_peer_id: string; // The peer ID in the IPFS network
    ipfs_peer_addresses: string[]; // Array of addresses at which the peer can be reached
    ...
    attempt_count: number; // Number of attempts made to change the pin status
    priority_pin: boolean; // Indicates whether this is a priority pin
}
...
interface PinStatus {
    ...
    peer_map: { [key: string]: PeerData }; // Map of peer data, with peer IDs as keys
}

interface RootObject {
    pinStatus: PinStatus; // The pin status object
}
```
---
