...
- **configuration:** An object with the following parameters:
  1.  `configuration.name`
      - Represents the name of the project on which you are pinning the CID.
  2.  `configuration.organizationId`
...
```js
const pinStatus: pinStatusRes = await client.getCIDStatus(CID);
```

Function `getCIDStatus` takes just one parameter:

- **CID** - (string) - The id of the bucket.

**Response**

```js

interface pinStatusRes {
  pinStatus: string;
}
```
---
