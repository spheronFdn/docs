...
- **configuration:** An object with the following parameters:
  ...
  - `configuration.inBackground` (string) - **(optional)** - when set to true, the pinning will processed in the background. Using the `uploadId` from the response, the status can be checked via [getUpload](/sdk/storage-v2/#get-upload) function.
