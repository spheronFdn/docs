...
#### Migrate Buckets From Web Organization

Used to migrate the Buckets and their Uploads from Web App organization to Storage organization.
...
Executing the migration will move the following data from webapp to storage organization:
...
const { numberOfBuckets, numberOfUploads } =
  await client.migrateWebAppOrgToStorage(
    storageOrganizationId,
    webAppOrganizationId
  );
```

Function `migrateWebAppOrgToStorage` takes two parameter:
...
- **webAppOrganizationId** - (string) - The id of the webapp organization from the the data will be moved
