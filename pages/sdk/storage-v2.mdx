...
Only uploads in `Unpinned` status can be pinned. The storage used will be increased by the upload size after the upload is pinned.
...
Only uploads in `Pinned` status can be unpinned. The storage used will be decreased by the upload size after the upload is unpinned.
