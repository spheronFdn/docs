...
- To migrate the buckets of your Static Site organization to your new Storage organization, please refer to the [migrateStaticSiteOrgToStorage](/sdk/storage-v2/#migrate-buckets-from-static-site-organization) method in our documentation.

- By following the migration process, you can seamlessly transition to the latest version of the SDK and leverage the enhanced features it offers while maintaining your existing data.
...
```
CHANGED FUNCTIONS:
getBucket, getBucketUploadCount, getBucketUploads, getOrganizationUsage, getUpload, all the ipns ones

New Methods:
- getOrganizationBuckets
- getOrganizationBucketCount
- migrateStaticSiteOrgToStorage
...
> The responses interface changes a bit, the uploadDirectory is an object
...
From version `2.0.0` of `@spheron/storage`, the SDK will only work with the Storage organization. Using this method you can migrate the Buckets and Uploads you have used before.
Executing the migration will move the following data from static site to storage organization:
...
<Callout type="info">
  **NOTE:** Important: After you migrate the data it is not possible to revert
  it.
</Callout>
...
- **staticSiteOrganizationId** - (string) - The id of the webapp organization from the the data will be moved
...
> Changes, there are a couple of new properties, storageFilecoin, hns and ens domains ( not sure why they were missing) and optimized images
...
> Difference is no domains here, user can use the method to get domains
...
> Changes, here we only return the total number
...
> The status changed, and the uploadDirectory structure
...
<Callout type="info">
  **NOTE:** Domains need to be enabled for the organization before you can use
  them. Contact us at discord to enable them.
</Callout>
