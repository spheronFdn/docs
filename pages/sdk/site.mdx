...
- `environmentVariables`: an object that represent environment variables that should be used when deploying. If new variables are passed, they will automatically be added to the project's environment variables. Besides this variables, the variables of the Project will also be added to the deployment process.
- `provider`: the provider of the repository.
- `branch`: the branch of the deployment.
- `protocol`: the protocol on which the deployment should be uploaded. Supported protocols are:
...
- `configuration`: the configuration that should be used for the deployment. Checkout [configuration interface](/sdk/site/#updateconfiguration) for more details.
