...
interface UsageWithLimits {
  ...
  usedParallelUploads?: number; // the number of uploads are in progress for an organization
  parallelUploadsLimit?: number; // the number on how many parallel uploads an organization can have
...
Used to archive a project. When a project is archived new deployments cannot be started for it.
...
Used to get the build directory structure of the deployment.

```js
const buildDirectory: string[] = await spheron.deployments.getBuildDirectory(
  deploymentId
);
```
...
Returns the build directory structure.
