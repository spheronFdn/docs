...
6. Create new port mapping. Add the container port, and Select the exposed port you want to map it to.
7. Add environment variables if any. Use the **Secret Key** toggle if the value is a secret key. When you enable the secret key toggle, it will not be saved in the
   database.
8. Select your preferred region if any. If you do not add a region, the container will be deployed in any region.
